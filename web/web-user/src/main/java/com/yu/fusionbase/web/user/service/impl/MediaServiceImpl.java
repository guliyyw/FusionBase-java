...
public class MediaServiceImpl implements MediaService {
    ...
    public MediaVO uploadMedia(String albumId, MultipartFile file, MediaUploadDTO dto) throws IOException, ServerException, InsufficientDataException, ErrorResponseException, NoSuchAlgorithmException, InvalidKeyException, InvalidResponseException, XmlParserException, InternalException {
        ...
        if (isImageFile(file) || isVideoFile(file)) {
            thumbnailInfo = generateThumbnail(mediaId, fileBytes, file.getContentType(), objectName, extension);
     ...
     * 生成缩略图（支持图片和视频）
     */
    public ThumbnailInfo generateThumbnail(String mediaId, byte[] fileBytes, String contentType, String objectName, String extension) {
        try {
            String thumbnailPath = "";

            if (isImage(contentType)) {
                return generateImageThumbnail(fileBytes, objectName, extension);
            } else if (isVideo(contentType)) {
                return generateVideoThumbnail(fileBytes, objectName);
            }
        } catch (Exception e) {
     ...
     * 生成图片缩略图
     */
    private ThumbnailInfo generateImageThumbnail(byte[] fileBytes, String objectName, String extension)
     ...
     * 生成视频缩略图（使用FFmpeg）
     */
    private ThumbnailInfo generateVideoThumbnail(byte[] fileBytes, String objectName)
